...
# The full set of devices we expect on the board. main.py runs YOLO every
# frame at start-up and will not "lock" the detection until ONE frame shows
# exactly this set, the same way the ArUco markers accumulate until all four
# are present before the homography locks. (The board is static, so once one
# clean frame shows everything, that frame is trustworthy.)
