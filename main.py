...
#
#   Instance numbers (the "4" in Terminal_2_4) are kept stable across
#   re-detections by an InstanceRegistry (see terminal_map.py), which is
#   created ONCE for the whole session below and reused on every re-detect.
#
...
def main():
    ...
    # One instance registry for the WHOLE session. This is what keeps a
    # device's instance number (the "4" in Terminal_2_4) attached to that
    # physical device instead of to its rank in a sorted list, across every
    # re-detect ('o'). Create this ONCE here, not inside the loop and not
    # inside the 'o' handler below - a registry created fresh on every
    # re-detect would reset all the numbering and bring the original bug
    # right back.
    instance_registry = tm.InstanceRegistry(config.DEVICE_MATCH_MAX_DISTANCE,
                                            config.DEVICE_ROW_TOLERANCE)
    ...
    while True:
        ...
        # --- Stage 2: run YOLO once, then stamp terminals onto the devices ---
        if detection_on and not components_detected:
            ...
            print(f"Detected components: {counts}")
        ...
        if key == ord('q'):
            ...
            components_detected = False     # forces a re-run + re-stamp next frame
            print("Re-running component detection...")
